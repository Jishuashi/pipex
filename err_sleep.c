...
/*   Updated: 2026/02/19 15:55:43 by hchartie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * Execute cmd if infile for cmd 1 is ok 
 * and out file for cmd 2 is ok
...
void	err_sleep(char *cmd1, char *cmd2, char	*in, char *out)
{
	...
	pid[0] = -1;
	if (ft_strnstr(cmd1, "sleep", ft_strlen(cmd1)) && access(in, F_OK) == 0)
		pid[0] = ft_execute_sleep(cmd1);
	pid[1] = -1;
	if (ft_strnstr(cmd2, "sleep", ft_strlen(cmd2)) && access(out, F_OK) == 0)
